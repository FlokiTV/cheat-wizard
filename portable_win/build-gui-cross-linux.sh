...
"$LLD_LINK" /entry:guiCRTStartup /subsystem:windows /nodefaultlib /machine:x64 /opt:ref /opt:icf /out:"$OUT/Cheat Wizard.exe" "$TMP/cw-gui.obj" "$TMP/kernel32.lib" "$TMP/user32.lib" "$TMP/gdi32.lib"

"$ROOT/portable_win/audit-gui-imports.sh" "$OUT/Cheat Wizard.exe"
